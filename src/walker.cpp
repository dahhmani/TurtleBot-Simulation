...
TurtlebotWalker::TurtlebotWalker(double ds, double ts) {
  ...
  scan_sub = nh.subscribe<sensor_msgs::LaserScan>(
      "/scan", 10, &TurtlebotWalker::lidarCallback, this);
  near_obstacle = false;
  front_clear = true;
  obstacle_thresh = 0.4;
  drive_speed = ds;
...
void TurtlebotWalker::lidarCallback(
  ...
  front_cone =
      std::vector<double>(msg->ranges.end() - cone_angle, msg->ranges.end());
